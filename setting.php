...
if (isset($_POST['verify'])) {
	$ffmpeg1 = !empty($_POST['ffmpeg']) ? checkin($_POST['ffmpeg']): checkin($ffmpeg) ;
	$user1 = !empty($_POST['user']) ? md5(checkin($_POST['user'])) : checkin($user);
	$passlama = !empty($_POST['pass']) ? md5(checkin($_POST['pass'])): '';
	if ($pass == md5($passlama)){
		$passbaru = isset($_POST['pass1']) ? md5(checkin($_POST['pass1'])):checkin($pass);
		$pesan = 'pass error';
	} else {
		$passbaru = $pass;
	}
	$ffmpeg10bit1 = !empty($_POST['ffmpeg10bit']) ? checkin($_POST['ffmpeg10bit']) : checkin($ffmpeg10bit)  ;
	$site1 = !empty($_POST['install']) ? checkin($_POST['install']) : checkin($site);
...
echo '<div class="div-div">';
echo '<p>Yukihana 1.0.1a (Ryuunosuke)</p>';
